<template>
    <div class="container mx-auto px-4 py-6">
      ...
      <div class="mt-6">
        <!-- Published Events -->
        <div v-if="activeTab === 'published'" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div
        ...
        <p v-if="activeTab === 'published' && publishedEvents.length === 0" class="text-gray-500">No published events available.</p>
  
        <!-- Draft Events -->
        <div v-if="activeTab === 'draft'" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div
          ...
          >
            <h3 class="text-lg font-bold text-gray-800">{{ event.title }}</h3>
            <p class="text-sm text-gray-500">{{ event.description }}</p>
            <p class="mt-2 text-sm text-gray-500">Tasks Completed: {{ event.completedTasks }}/{{ event.totalTasks }}</p>
            <button
        ...
        <p v-if="activeTab === 'draft' && draftEvents.length === 0" class="text-gray-500">No draft events available.</p>
        ...
        <div v-if="activeTab === 'pending'" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            <h3 class="text-lg font-bold text-gray-800">{{ event.title }}</h3>
            <p class="text-sm text-gray-500">{{ event.description }}</p>
            <p class="mt-2 text-sm text-yellow-600 font-medium">Status: Pending Approval</p>
          </div>
        </div>
        <p v-if="activeTab === 'pending' && pendingEvents.length === 0" class="text-gray-500">No pending events available.</p>
        ...
        <p v-if="activeTab === 'past' && pastEvents.length === 0" class="text-gray-500">No past events available.</p>
      </div>
    </div>
  </template>
  ...
  const events = ref([
    ...
    },
    {
      ...
      description: 'Learn about the latest trends in digital marketing.',
      status: 'rejected',
      completedTasks: 8,
      totalTasks: 10,
      endDate: '2025-03-15',
  ...
  // Categorize events
  const publishedEvents = ref(events.value.filter((event) => ['approved', 'rejected'].includes(event.status)));
  const draftEvents = ref(events.value.filter((event) => event.status === 'draft'));
  const pendingEvents = ref(events.value.filter((event) => event.status === 'pending'));
  const pastEvents = ref(events.value.filter((event) => new Date(event.endDate) < new Date()));
